...
#[serial]
async fn test_screeners() -> Result<(), Box<dyn Error>> {
    let logfile = "testdata/test_screeners.txt".to_string();
    ...
    let tickers = super::models::tickers::TickersBuilder::new()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn test_ticker_data() -> Result<(), Box<dyn Error>> {
    let logfile = "testdata/test_ticker_data.txt".to_string();
    ...
    for i in 0..symbols.len() {
        ...
        let mut ticker = super::models::ticker::TickerBuilder::new()
            .ticker(&stock_symbol)
        ...
        } else if chrono::Utc::now().weekday() == chrono::Weekday::Sun {
            chrono::Utc::now()
                .date_naive()
                .checked_sub_days(chrono::Days::new(2)).unwrap()   // Last Friday
        } else {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn test_portfolio() -> Result<(), Box<dyn Error>> {
    let logfile = "testdata/test_portfolio.txt".to_string();
